...
# install-ohs-maintenance-tool.sh
#
# Installs a permanent repository-maintenance command:
#
#   uv run ohs-maintain --check
#   uv run ohs-maintain --fix
#
# The tool performs safe, targeted cleanup and creates timestamped backups
# before replacing or removing files.
...
mkdir -p src/ohs tests/maintenance

cat > src/ohs/maintenance.py <<'PY'
...
class Maintainer:
    ...
    def normalize_ohs001(self) -> None:
        ...
        missing = [
            filename
            for filename in EXPECTED_OHS001
...
"""
    ...
    def check_roadmap(self) -> None:
        ...
        stale = [
            model
            for model in ("Ousdal Hus 80", "Ousdal Hus 100")
            if model in text
        ]
...
"""
    ...
    def check_root_contract(self) -> None:
        ...
        missing = [
            ...
            if name in text and not (self.root / name).exists()
        ]
        if missing:
            self.report.warn(
                area,
                "ARCHITECTURE.md references missing root files: "
                + ", ".join(missing),
            )
        else:
...
text = path.read_text(encoding="utf-8")
entry = 'ohs-maintain = "ohs.maintenance:main"'
...
if entry not in text:
    ...
    print("[+] Added ohs-maintain CLI to pyproject.toml")
else:
    print("[=] ohs-maintain CLI already configured")
PY

cat > tests/maintenance/test_maintenance.py <<'PY'
from pathlib import Path
...
touch .gitignore
grep -qxF ".ohs-maintenance-backup/" .gitignore || {
  printf '\n# Local backups created by ohs-maintain --fix\n.ohs-maintenance-backup/\n' >> .gitignore
}
...
echo
echo "[✓] Permanent maintenance tool installed."
echo
echo "First run:"
